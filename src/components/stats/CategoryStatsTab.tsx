...
export function CategoryStatsTab({ 
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 카테고리별 비중 원그래프 (크기 키움, 하얀 윤곽선 제거, 부드러운 파스텔톤) */}
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-4 sm:p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 sm:mb-6">
          <div>
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
              우주하마가 방송에서 플레이한 게임들의 장르 및 카테고리 분포 비율입니다.
            </p>
          </div>
          <span className="text-xs font-semibold px-3 py-1 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700 self-start sm:self-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          ...
          <div className="lg:col-span-6 h-[320px] sm:h-[360px] relative flex items-center justify-center">
            {isActive && (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none text-center">
              ...
              <span className="text-base sm:text-lg font-bold text-zinc-900 dark:text-white max-w-[130px] truncate px-1">
                {topCategory?.name || '종합'}
              </span>
          ...
          <div className="lg:col-span-6 space-y-2">
            <h5 className="text-xs font-semibold text-zinc-400 dark:text-zinc-500 mb-2">
              주요 카테고리 비중 순위
            </h5>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {chartData.map((item) => (
                <div
                  key={item.name}
                  className="flex items-center justify-between p-2.5 rounded-xl border border-zinc-200/60 dark:border-zinc-800/60 bg-zinc-50/50 dark:bg-zinc-950/40 text-xs"
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span className="font-bold text-zinc-900 dark:text-zinc-100 min-w-[38px] text-right">
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-4 sm:p-6 shadow-sm">
          ...
          <div className="flex items-center gap-2 w-full lg:w-auto">
            ...
            <div className="relative flex-1 lg:w-64">
              ...
              <input
                ...
                className="w-full pl-8 pr-7 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 text-xs sm:text-sm dark:text-zinc-200"
              />
              {searchTerm && (
                <button
            ...
            <div className="relative shrink-0">
              <select
                ...
                className="pl-3 pr-7 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 text-xs sm:text-sm appearance-none dark:text-zinc-200 font-medium cursor-pointer"
              >
        ...
        {/* 순위표 테이블 (모바일 글씨 크기 줄이고 최소한의 스크롤로 볼 수 있게 최적화) */}
        <div className="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-800 custom-scrollbar">
          <table className="w-full text-left text-xs sm:text-sm whitespace-nowrap">
            <thead className="bg-zinc-50 dark:bg-zinc-950 text-zinc-500 dark:text-zinc-400 font-semibold border-b border-zinc-200 dark:border-zinc-800 text-[11px] sm:text-xs">
              <tr>
                <th className="px-2 sm:px-4 py-2.5 sm:py-3 w-12 sm:w-16 text-center">순위</th>
                <th className="px-2.5 sm:px-4 py-2.5 sm:py-3">카테고리</th>
                <th 
                  onClick={() => toggleSort('count')}
                  className="px-2.5 sm:px-4 py-2.5 sm:py-3 cursor-pointer hover:text-purple-600 transition-colors"
                >
                ...
                <th 
                  ...
                  className="px-2.5 sm:px-4 py-2.5 sm:py-3 text-right cursor-pointer hover:text-purple-600 transition-colors"
                >
                ...
                filteredCategories.map((cat, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-2.5 sm:px-4 py-2.5 sm:py-3">
                        <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 max-w-xs sm:max-w-md lg:max-w-lg">
                          {visibleGames.map((g, gIdx) => (
                            <button
                              ...
                              className="inline-flex items-center gap-1 text-[10px] sm:text-[11px] px-1.5 sm:px-2 py-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800 hover:bg-purple-100 dark:hover:bg-purple-900/30 text-zinc-700 dark:text-zinc-300 hover:text-purple-600 dark:hover:text-purple-300 font-medium border border-zinc-200/70 dark:border-zinc-700/70 hover:border-purple-300 dark:hover:border-purple-700 truncate max-w-[130px] sm:max-w-[160px] transition-colors cursor-pointer group/btn"
                              title={g.latestDate ? `${g.name} (${g.count}회) - 클릭 시 ${g.latestDate} 방송 기록 달력으로 이동` : `${g.name} (${g.count}회)`}
                            >
                              <span className="truncate">{g.name}</span>
                              <span className="text-[9px] sm:text-[10px] font-bold text-purple-600 dark:text-purple-400">
                                {g.count}
                              </span>
                              {g.latestDate && (
                          ...
                          {cat.games.length > 3 && (
                            <button
                              ...
                              className="inline-flex items-center text-[10px] sm:text-[11px] px-1.5 sm:px-2 py-0.5 rounded-md bg-zinc-200/60 dark:bg-zinc-700/60 hover:bg-zinc-300 dark:hover:bg-zinc-600 text-zinc-600 dark:text-zinc-200 font-medium transition-colors cursor-pointer"
                            >
